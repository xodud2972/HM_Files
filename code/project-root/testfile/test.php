       ...
       if(isset($colData[$colIndex]['col4'])){
			$querySelectDataList = sprintf(
					'SELECT c1.cs_num, getcomkrname(c1.cs_type_new1) AS cs_type_new1 , getcomkrname(c1.cs_type_new2) AS cs_type_new2, 
						...
						getcomkrname(c2.m_nm1) AS m_nm
					FROM t_customer c1
					INNER JOIN(
								SELECT * FROM (
												SELECT cs_seq, cs_m_id, pay_date, pay_price, good_type2, if(m_nm="302",good_type2,m_nm) as m_nm1
												from t_contract
...
// echo iconv('utf-8', 'euc-kr', $querySelectDataList);
// echo '/////////';
//        $result = que($querySelectDataList);
...
if(isset($colData[$colIndex]['col3']) !== '' && isset($colData[$colIndex]['col2']) !== ''){
	$querySelectDataList = sprintf(
		...
		mg_tel1, mg_tel2, mg_tel3, mg_email, IFNULL(round(sum(pay_price)/%d,0),0) AS pay_price, getcomkrname(if(m_nm="302",good_type2,m_nm)) as m_nm
	FROM t_customer_md t1
	INNER JOIN t_customer t2 ON t1.cs_seq = t2.cs_seq
	left JOIN (
					SELECT max(cs_seq) AS "cs_seq", sum(pay_price) AS "pay_price", pay_date 
					FROM t_contract 
					WHERE pay_date BETWEEN "%s" AND "%s"
					AND cs_m_id = "%s"
					AND del_date IS NULL
				)t3 ON t3.cs_seq = t2.cs_seq
	WHERE cs_m_id = "%s"
	AND if(m_nm="302",good_type2,m_nm) = "%s"
	GROUP BY cs_num, cs_m_id
	ORDER BY cs_m_id, m_nm',
	$divide, $sDate, $eDate, $colData[$colIndex]['col3'], $colData[$colIndex]['col3'], $colData[$colIndex]['col2']);