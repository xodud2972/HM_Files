...
SELECT
  c1.cs_num,
  getcomkrname(c1.cs_type_new1) AS cs_type_new1,
  getcomkrname(c1.cs_type_new2) AS cs_type_new2,
  c1.cs_nm,  c1.url AS url,  c1.mg_nm,
  c1.mg_cell1,c1.mg_cell2, c1.mg_cell3,
  c1.mg_tel1,  c1.mg_tel2,  c1.mg_tel3,c1.mg_email,
  round(SUM(c2.pay_price) / 3, 0) AS pay_price,
  getcomkrname(c2.m_nm) AS m_nm1,m_nm
FROM
  t_customer c1
  INNER JOIN(
    SELECT
      cs_seq,cs_m_id,m_nm,pay_date,pay_price
    from
      t_contract
    WHERE
      cs_m_id = "apple7878"
      AND pay_date BETWEEN "2010-10-01"AND "2022-12-31"
...
ORDER BY
  ...
  -- 광고주명 / 매체명 / 광고계정 / 인입월 / 마케터명 / 사업자번호
  -- ceo_nm   	  /  m_nm 	/ cs_m_id 	/ reg_date 	/ em_seq 	/ cs_num
  -- t_customer /contract	/contract	/contract	/contact 	/ contract
  
-- 사업자번호 / 인입월 / 담당자 / 계정명 / 매체 /
  --         em_seq        cs_num로조인한contract의m_nm
...
FROM t_customer_md md INNER JOIN (
													SELECT cs_seq, cs_num 
												  	FROM t_customer
												  	WHERE reg_date BETWEEN '2022-01-01' AND '2022-01-31'
													AND del_date is null
											) cus ON (md.cs_seq = cus.cs_seq)
WHERE getcomkrname(IF(md.m_nm='302', md.good_type2, md.m_nm)) IS NOT NULL 
AND  md.reg_date BETWEEN '2022-01-01' AND '2022-01-31'
AND md.del_date IS NULL
AND cus.cs_num NOT IN (
								SELECT t_customer.cs_num
							   FROM t_customer_md inner JOIN t_customer ON (t_customer_md.cs_seq = t_customer.cs_seq) 
							   WHERE t_customer_md.m_nm = '808' 
								AND t_customer_md.del_date IS NULL6
							)
ORDER BY md.reg_date			

SELECT COUNT(*) FROM t_project_content_1031